            ...
            int rdvport)
{
   ...
   si_other.sin_port = htons(rdvport);
   si_other.sin_addr.s_addr = inet_addr(rdvaddr);
   ...
   while (1)
   {
      ...
      if (recvfrom(s, (char *)&buf, sizeof(buf), 0, (struct sockaddr*)(&si_other), &slen)==-1)
         diep("recvfrom");
